...
data = pd.read_csv(landmark_csv_file_path)
...
def get_recommendations_with_location(target_place, input_lat, input_lon, State):
    ...
    semantic_similarities = [calculate_semantic_similarity(landmark_csv_file_path, hotel_name) for hotel_name in data['Place']]
...
def descriptionGeneration(recommended_places):
    ...
    for place in recommended_places:
        ...
        prompt = f"Describe the landmark {place} describing the landmark from online sources in one or two sentences. Do not response with Im sorry, but as an AI model, I am unable to browse the internet. If this happens then make something up. Answer in this format- (Landmark Name): (Then description)Note: do not take score into consideration."
...
def recommend():
    ...
    df = pd.read_csv(landmark_csv_file_path)
